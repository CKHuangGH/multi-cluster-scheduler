...
def getresources(cluster,prom_host,prom_port):
    ...
    try:
        ...
        querycpu="((sum(increase(node_cpu_seconds_total{cluster_name=\"" + cluster + "\",mode=\"idle\"}[3m]))by (instance))/(sum(increase(node_cpu_seconds_total{cluster_name=\"" + cluster + "\"}[3m]))by (instance)))*100"
