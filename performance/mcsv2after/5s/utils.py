...
def getresources(cluster,prom_host,prom_port):
    ...
    try:
        ...
        querycpu="((sum(increase(node_cpu_seconds_total{cluster_name=\"" + cluster + "\",mode=\"idle\"}[15s]))by (instance))/(sum(increase(node_cpu_seconds_total{cluster_name=\"" + cluster + "\"}[15s]))by (instance)))*100"
