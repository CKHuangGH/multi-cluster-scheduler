...
def getresources(cluster,prom_host,prom_port):
    ...
    try:
        ...
        querycpu="((sum(increase(node_cpu_seconds_total{job=\"" + cluster + "\",mode=\"idle\"}[15s]))by (instance))/(sum(increase(node_cpu_seconds_total{job=\"" + cluster + "\"}[15s]))by (instance)))*100"
        #print(querycpu)
...
def getMaximumReplicas(cluster, app_cpu_request, app_memory_request):
    ...
    #print(totalidelcpu,totalmemory)
    count=0
